...
function filterPirates (pirates) {
    ...
    let birthYear = 0, loc = '', cache=[];
    
    for (let p in pirates) {
      let obj = {}
      birthYear = 2020 - pirates[p].age;
      birthYear = birthYear.toString();
      loc = (pirates[p].location == 'Jakarta') ? 'JKT': pirates[p].location == 'Bogor' ? 'BGR'
            : pirates[p].location == 'Depok' ? 'DPK': pirates[p].location == 'Bekasi' ? 'BKS':undefined

      obj.name = pirates[p].name;
      // ORIGINAL CODE
      // obj.ID = `${birthYear.toString()[birthYear.toString().length-2]}${birthYear.toString()[birthYear.toString().length-1]}-${loc}`
      // if (indexOf(pirates[p].name,'.') == -1 && loc !== undefined) {cache.push(obj);}
      obj.ID = `${birthYear.substr(birthYear.length-2,birthYear.length)}-${loc}`
      if (pirates[p].name.indexOf('.') == -1 && loc !== undefined) {cache.push(obj)}
    }
